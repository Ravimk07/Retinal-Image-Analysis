# -*- coding: utf-8 -*-
"""
Created on Sun Aug 26 23:26:17 2018
...
def get_im_resizer(path):
    img = cv2.imread(path)
    resized_img = cv2.resize(img, (299, 299), cv2.INTER_LINEAR)
    return resized_img
...
def load_data():
    ...
    for j in range(4):
        ...
        path = os.path.join(r'F:\Ravi\Infrred\dataset', 'Grade ' + str(j), '*.jpg')
...
def preprocess_data():
    cache_path = os.path.join('cache', 'data_r_' + str(299) + '_c_' + str(299) + '_t_' + str(3) + '.dat')
    ...
    data = np.reshape(data,(336, 299, 299, 3))
